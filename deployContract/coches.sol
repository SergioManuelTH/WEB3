...
contract Coches{
    address owner;
    uint256 precio;
    uint256[] identificadores;//id de cada coche dado de alta
    ...
    constructor (uint _precio) {
        owner= msg.sender;
        precio= _precio;
    }

    function AddCoche(uint256 _id, string memory _marca, uint32 _caballos, uint32 _kilometros) 
             external precioFiltro(msg.value) payable{
    ...
    function GetNumIdentificadores() external view returns(uint256){
